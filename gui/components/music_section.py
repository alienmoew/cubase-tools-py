...
class MusicSection(BaseComponent):
    ...
    def create(self):
        ...
        # Section frame - improved design with better colors
        self.container = CTK.CTkFrame(self.parent, corner_radius=5, border_width=1, border_color="#505050", fg_color="#252525")
        
        # Header với title và toggle
        self._create_header()
        
        # Content frame - compact padding
    ...
    def _create_header(self):
        """Tạo header với title và bypass toggle - improved design."""
        header_frame = CTK.CTkFrame(self.container, fg_color="#202020", corner_radius=3)
        header_frame.pack(fill="x", padx=2, pady=2)
        
        # Title - dời về lề trái
        ...
        title_label.pack(side="left", padx=6, pady=3)
        
        # SoundShifter Toggle (right side) - improved design
        toggle_container = CTK.CTkFrame(header_frame, fg_color="transparent")
        toggle_container.pack(side="right", padx=6)
    ...
    def _create_volume_controls(self, parent):
        """Tạo volume controls với màu tím - improved design."""
        volume_container = CTK.CTkFrame(parent, fg_color="#2A2A2A", corner_radius=4, border_width=1, border_color="#8E24AA")
        volume_container.pack(fill="x", pady=(0, 3), padx=2)
        
        # Row chứa label và value trong cùng một hàng - label dời về lề trái
        top_row = CTK.CTkFrame(volume_container, fg_color="transparent")
        top_row.pack(fill="x", pady=(3, 0), padx=4)
        
        # Label bên trái - dời về lề trái
        ...
        # Buttons row - improved design
        btn_frame = CTK.CTkFrame(volume_container, fg_color="transparent")
        btn_frame.pack(pady=(0, 3), padx=4)
        
        # Decrease button - improved styling
        ...
        btn_decrease.pack(side="left", padx=(0, 2))
        
        # Mute toggle - improved styling
        ...
        self.mute_toggle_btn.pack(side="left", padx=(0, 2))
        
        # Increase button - improved styling
    ...
    def _create_tone_controls(self, parent):
        """Tạo tone nhạc controls với màu xanh dương - improved design."""
        tone_nhac_container = CTK.CTkFrame(parent, fg_color="#2A2A2A", corner_radius=4, border_width=1, border_color="#1976D2")
        tone_nhac_container.pack(fill="x", pady=(0, 3), padx=2)
        
        # Row chứa label và value trong cùng một hàng - label dời về lề trái
        top_row = CTK.CTkFrame(tone_nhac_container, fg_color="transparent")
        top_row.pack(fill="x", pady=(3, 0), padx=4)
        ...
        self.soundshifter_value_label.pack(side="right")
        ...
        # Nút Reset/Bình thường - improved styling
        ...
        btn_normal.pack(side="left", padx=(0, 2))
    ...
    def _create_transpose_controls(self, parent):
        """Tạo transpose controls với màu tím - improved design."""
        transpose_frame = CTK.CTkFrame(parent, fg_color="#2A2A2A", corner_radius=4, border_width=1, border_color="#8E24AA")
        transpose_frame.pack(fill="x", pady=(0, 3), padx=2)
        
        # Label ở trên - dời về lề trái
        label_frame = CTK.CTkFrame(transpose_frame, fg_color="transparent")
        label_frame.pack(fill="x", pady=(3, 0), padx=4)
        ...
        self.btn_pitch_young.pack(side="left")
    ...
    def update_mute_display(self, is_muted):
        ...
        if self.mute_toggle_btn:
            # Thay đổi màu sắc khi tắt tiếng để dễ nhận biết hơn
            if is_muted:
                self.mute_toggle_btn.configure(text="Bật", fg_color="#D32F2F", hover_color="#B71C1C", border_color="#F44336")
            else:
                self.mute_toggle_btn.configure(text="Tắt", fg_color="#8E24AA", hover_color="#7B1FA2", border_color="#AB47BC")
    
    def update_soundshifter_display(self, value):
        """Cập nhật hiển thị soundshifter value theo tone (±2 value = ±1 tone) - improved format."""
        if self.soundshifter_value_label:
            # Chuyển đổi value sang tone: mỗi 2 value = 1 tone
            tone = value / 2.0
            
            # Format hiển thị ngắn gọn với màu sắc tùy thuộc vào giá trị
            if tone == 0:
                display = "0"
                color = "#BBDEFB"  # Màu xanh dương nhạt
            elif tone > 0:
                display = f"+{tone:.1f}" if tone % 1 != 0 else f"+{int(tone)}"
                color = "#81C784"  # Màu xanh lá nhạt
            else:
                display = f"{tone:.1f}" if tone % 1 != 0 else f"{int(tone)}"
                color = "#FFAB91"  # Màu cam nhạt
            
            self.soundshifter_value_label.configure(text=display, text_color=color)
    
    def update_volume_display(self, value):
        """Cập nhật hiển thị volume value theo phần trăm với màu sắc tùy thuộc vào mức âm lượng."""
        if self.volume_value_label:
            # Thay đổi màu sắc dựa trên mức âm lượng
            if value == 0:
                color = "#EF5350"  # Đỏ cho âm lượng 0
            elif value < 30:
                color = "#FFCA28"  # Vàng cho âm lượng thấp
            elif value < 70:
                color = "#66BB6A"  # Xanh lá cho âm lượng trung bình
            else:
                color = "#EF5350"  # Đỏ cho âm lượng cao
                
            self.volume_value_label.configure(text=f"{value}%", text_color=color)
    ...
    def _highlight_pitch_button(self, selected):
        """Highlight nút pitch đang được chọn với màu tím và hiệu ứng."""
        default_color = "#8E24AA"  # Tím đậm (base)
        active_color = "#D81B60"   # Hồng đậm (active)
        default_border = "#AB47BC"  # Viền tím nhạt
        active_border = "#F06292"   # Viền hồng nhạt
        
        # Reset tất cả về màu base
        if self.btn_pitch_old:
            self.btn_pitch_old.configure(fg_color=default_color, border_color=default_border)
        if self.btn_pitch_normal:
            self.btn_pitch_normal.configure(fg_color=default_color, border_color=default_border)
        if self.btn_pitch_young:
            self.btn_pitch_young.configure(fg_color=default_color, border_color=default_border)
        
        # Highlight nút được chọn
        if selected == 'old' and self.btn_pitch_old:
            self.btn_pitch_old.configure(fg_color=active_color, border_color=active_border)
        elif selected == 'normal' and self.btn_pitch_normal:
            self.btn_pitch_normal.configure(fg_color=active_color, border_color=active_border)
        elif selected == 'young' and self.btn_pitch_young:
            self.btn_pitch_young.configure(fg_color=active_color, border_color=active_border)
