...
class MainWindow:
    ...
    def __init__(self):
        ...
        app_name = self.default_values.get('system_volume_app_name', 'chrome.exe')
    ...
    def _batch_reset_autotune_parameters(self):
        ...
        try:
            ...
            if self.autotune_section and self.autotune_section.pitch_slider:
                default_transpose = self.default_values.get('transpose_default', 0)
                self.autotune_section.pitch_slider.set(default_transpose)
                self.autotune_section.update_transpose_value(default_transpose)
            
            print(f"✅ Ultra fast batch reset completed: {success_count}/{total_count} success")
    ...
    def _update_soundshifter_display(self):
        ...
        if self.music_section and self.music_section.soundshifter_value_label:
            ...
            self.music_section.soundshifter_value_label.configure(text=str(current_value))
        ...
        try:
            ...
            if success:
                ...
                if self.music_section and self.music_section.mute_toggle_btn:
                    current_text = self.music_section.mute_toggle_btn.cget("text")
                    if "M" == current_text:
    ...
    def _adjust_bass_instant(self, direction):
        ...
        try:
            ...
            self.vocal_section.bass_value_label.configure(text=f"Bass: {new_value}")
    ...
    def _adjust_treble_instant(self, direction):
        ...
        try:
            ...
            self.vocal_section.treble_value_label.configure(text=f"Treble: {new_value}")
    ...
    def _adjust_volume_mic_instant(self, direction):
        ...
        try:
            current_value = int(round(self.vocal_section.volume_mic_slider.get()))
    ...
    def _adjust_reverb_mic_instant(self, direction):
        ...
        try:
            current_value = int(round(self.vocal_section.reverb_mic_slider.get()))
    ...
    def _update_system_volume_display(self, percent):
        ...
        try:
            if self.music_section and self.music_section.volume_value_label:
                self.music_section.volume_value_label.configure(text=f"{percent}%")
        except Exception as e:
    ...
    def _initialize_system_volume_display(self):
        ...
        try:
            ...
            print(f"🔊 Initial system volume: {current_percent}% (muted: {is_muted})")
        except Exception as e:
